...
try:
    # Import the VPA pipeline components
    from livekit.plugins import noise_cancellation
    from livekit.plugins import deepgram, openai, cartesia, silero
    from livekit.plugins.turn_detector.multilingual import MultilingualModel
    
    # Import our voice agent implementation
    from voice_agent import voice_agent_entrypoint, VoiceAssistant, create_voice_agent_session
except ImportError as e:
    logger.error(f"Failed to import required packages: {e}")
    logger.error("Please install the missing packages: pip install 'livekit-agents[deepgram,openai,cartesia,silero,turn-detector]~=1.0' 'livekit-plugins-noise-cancellation~=0.2'")
    sys.exit(1)
...
async def entrypoint(ctx: agents.JobContext):
    ...
    # Check for required API keys for the VPA pipeline
    required_api_keys = {
    ...
    missing_keys = [key for key, value in required_api_keys.items() if not value or value == "your_" + key.lower()]
    if missing_keys:
        logger.error(f"Missing required API keys: {', '.join(missing_keys)}")
        logger.warning("Agent session creation may fail without valid API keys")
    
    # Create the session with the Voice Processing Agent (VPA) pipeline
    try:
        logger.info("Using LiveKit URL: %s", os.environ.get("LIVEKIT_URL"))
        
        if missing_keys:
            logger.error("Creating session with missing API keys (may fail)")
        
        # Create a VoiceAssistant with our persona configuration
        assistant = VoiceAssistant(instructions=GLOBAL_PERSONA_CONFIG.instructions)
        ...
        logger.info("Created agent session with VPA pipeline")
        ...
        # Register tools if enabled (will skip this for voice-only agents)
        if GLOBAL_ENABLE_TOOLS:
        ...
        logger.error(f"Failed to create VPA pipeline: {e}")
        logger.warning("Agent session creation failed. Check API keys and dependencies.")
        return
...
if __name__ == "__main__":
    ...
    parser.add_argument('connect', nargs='?', help='Connect command')
    parser.add_argument('--room', type=str, help='LiveKit room to join')
    
    # Parse known args without raising error for unknown args
    args, remaining = parser.parse_known_args()
    
    # Handle the connect command
    if hasattr(args, 'connect') and args.connect == 'connect':
        if not args.room:
            logger.error("Room name required: --room <room_name>")
            sys.exit(1)
        logger.info(f"Connecting to room: {args.room}")
        # Make sure room is passed along to the LiveKit CLI
        remaining = ['connect', '--room', args.room] + [arg for arg in remaining if not arg.startswith('--room')]
