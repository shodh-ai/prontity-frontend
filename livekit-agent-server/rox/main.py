...
class RoxAgent(Agent):
    ...
    def __init__(self, page_path="roxpage") -> None:
        ...
        self._latest_student_context: Optional[Dict[str, Any]] = {"user_id": "default_init_user"}
        self._latest_session_id: Optional[str] = f"init_session_{uuid.uuid4().hex[:8]}_{int(time.time())}"
        self._interaction_service_registered = False
        self._room: Optional[rtc.Room] = None # Ensure _room is typed
        self.custom_llm_bridge = CustomLLMBridge(rox_agent_ref=self)
        logger.info(f"RoxAgent.__init__ called for page_path: {page_path}.")
...
async def entrypoint(ctx: JobContext):
    ...
    try:
        ...
        main_agent_session = agents.AgentSession(
            ...
            llm=rox_agent_instance.custom_llm_bridge, # Use bridge from agent instance
        ...
        # Instantiate and register RPC service
