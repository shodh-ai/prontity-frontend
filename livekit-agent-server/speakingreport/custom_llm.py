    ...
    async def _chat_context_manager(self, chat_ctx: ChatContext, tools, tool_choice):
        ...
        try:
            ...
            async def response_generator():
                ...
                # Debug the chat context structure
                logger.debug(f"CustomBridge received chat context of type: {type(chat_ctx)}")
                
                # Extract messages from the context - be more robust
                messages = []
                ...
                try:
                    ...
                    # Try to access the messages attribute directly
                    elif hasattr(chat_ctx, 'messages'):
                        messages = getattr(chat_ctx, 'messages')
                        logger.debug(f"Got messages from chat_ctx.messages attribute")
