  ...
  const handlePerformUIAction = useCallback(
    async (rpcInvocationData: RpcInvocationData): Promise<string> => {
      ...
      try {
        ...
        console.log(
          `[DashRoxPage] Received action: ${request.actionType}`,
          request
        );
        ...
        console.error(
          "[DashRoxPage] Error handling Agent PerformUIAction:",
          innerError
        );
        const errMessage =
      ...
      <div style={{ display: "none" }}>
        <LiveKitSession
          ...
          onConnected={(connectedRoom, rpcAdapter) => {
            console.log(
              "LiveKit connected in DashRoxPage, room:",
              connectedRoom
            );
            liveKitRpcAdapterRef.current = rpcAdapter;
            console.log(
              "LiveKitRpcAdapter assigned in DashRoxPage:",
