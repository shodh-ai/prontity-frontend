...
const IconMicrophone = () => <span>🎤</span>;
const IconPause = () => <span>⏸</span>;
const IconDeviceFloppy = () => <span>💾</span>;
...
export default function EnhancedSpeakingTestPage() {
  ...
  const connectToDeepgram = useCallback(() => {
    ...
    // For Deepgram WebSocket API, configure it to match our actual audio format
    // Use the default audio settings to maximize compatibility
    const requestUrl = 'wss://api.deepgram.com/v1/listen?encoding=opus&channels=1&interim_results=true&punctuate=true&smart_format=true&model=nova-2';
    ...
    try {
      // Create a WebSocket with the correct protocol format
      // For Deepgram, the protocol is in the format 'token:{apiKey}'
      const socket = new WebSocket(requestUrl, [`token:${deepgramApiKey}`]);
      ...
      socket.onmessage = (message) => {
        ...
        if (data.type === 'Results') {
          ...
          if (transcript) {
            console.log('✅ Deepgram transcript extracted successfully:', transcript);
            handleSTTResult({ transcript_segment: transcript });
          } else {
            console.warn('⚠️ Could not find transcript in Deepgram response structure');
          }
        }
      };
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      // Setup Deepgram connection with API key in URL
      const deepgramApiKey = process.env.NEXT_PUBLIC_DEEPGRAM_API_KEY;
      if (!deepgramApiKey) {
      ...
      try {
        // Properly encode the API key for URL safety
        const encodedKey = encodeURIComponent(deepgramApiKey);
        
        // Log a masked version of the key for debugging
        const maskedKey = deepgramApiKey.length > 10 ? 
          `${deepgramApiKey.substring(0, 5)}...${deepgramApiKey.substring(deepgramApiKey.length - 5)}` : '***';
        console.log(`Using Deepgram API key (masked): ${maskedKey}`);
        
        // Create URL with API key as parameter - using simpler parameters
        const deepgramUrl = `wss://api.deepgram.com/v1/listen?encoding=webm&sample_rate=16000&interim_results=true&token=${encodedKey}`;
        ...
        deepgramSocket.onmessage = (message) => {
          try {
            const data = JSON.parse(message.data);
            console.log('Received data from Deepgram');
            
            // Extract transcript using optional chaining
        ...
        deepgramSocket.onerror = (event) => {
          ...
          alert('Failed to connect to Deepgram for transcription. Please try again later.');
        };
        
        deepgramSocket.onclose = (event) => {
          console.log(`Deepgram connection closed: [${event.code}] ${event.reason || 'No reason provided'}`);
        };
      } catch (error) {
        console.error('Error setting up Deepgram connection:', error);
      }
      ...
      recorder.ondataavailable = (e) => {
        ...
        if (e.data.size > 0) {
          ...
          // Also send to socket server if needed
  ...
  const stopRecording = () => {
    ...
    // Process the transcription for the editor
    if (liveTranscription.trim()) {
      setEditorContent(liveTranscription);
      console.log('Transcription added to editor:', liveTranscription);
      
      // Calculate word count
      const words = liveTranscription.trim().split(/\s+/).filter(word => word.length > 0);
