...
function SpeakingReportContent() {
  ...
  useEffect(() => {
    ...
    try {
      const storedTaskData = localStorage.getItem('speakingTask');
      if (storedTaskData) {
    ...
    const fetchTranscriptionData = async () => {
      try {
        ...
        const htmlTranscript = localStorage.getItem('speakingTranscriptHtml');
        if (htmlTranscript && htmlTranscript.length > 10) {
          ...
          let taskData = {};
          try {
            const taskDataJson = localStorage.getItem('speakingTask');
            if (taskDataJson) {
          ...
          setTranscriptionData({
            ...
            practiceDate: localStorage.getItem('speakingTranscriptTime') || new Date().toISOString(),
            topicId: topicId,
            taskId: taskId
          });
        ...
        const plainTranscript = localStorage.getItem('speakingTranscript');
        ...
        setTranscriptionError('No transcription found for this task. Please complete the speaking task first.');
      } catch (err) {
                ...
                {transcriptionLoading ? (
                  ...
                  <div className="bg-white min-h-[500px] border border-red-100 rounded-md p-4">
                    <div className="text-center">
                      ...
                      <p className="mt-4 text-gray-600">Tip: Make sure you're logged in and have completed a speaking task for this topic.</p>
                    </div>
                  </div>
                ) : (
                  <TiptapEditor
