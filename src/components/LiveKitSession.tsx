...
export class LiveKitRpcAdapter implements Rpc {
  constructor(
    ...
    // Custom timeout for RPC calls that we'll implement ourselves using Promise.race
    // This works around the limitation that LiveKit's performRpc doesn't accept a timeout parameter
    private timeout: number = 30000, // Default to 30 seconds for better reliability
  ) {}
  ...
  async request(service: string, method: string, data: Uint8Array): Promise<Uint8Array> {
    ...
    try {
      ...
      console.log(`RPC Request: To=${this.agentIdentity}, Method=${fullMethodName}, Timeout=${this.timeout}ms, Payload (base64)=${payloadString.substring(0,100)}...`);
      
      // Create a manual timeout promise
      const timeoutPromise = new Promise<never>((_, reject) => {
        setTimeout(() => {
          reject(new Error(`RPC timeout after ${this.timeout}ms`));
        }, this.timeout);
      });
      
      // The actual RPC call
      const rpcPromise = this.localParticipant.performRpc({
        destinationIdentity: this.agentIdentity,
        method: fullMethodName,
        payload: payloadString,
      });
      
      // Race between the RPC call and our timeout
      const responseString = await Promise.race([
        rpcPromise,
        timeoutPromise
      ]);
